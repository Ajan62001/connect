...
from connect.domain.models import CurrentUser, DocumentPage
...
async def feed(status: str | None = Query(default=None),
    ...
    enrichment_status (status chips in the UI). Carries the tenancy
    visibility predicate (shared OR mine)."""
    ...
    items, total = await doc_dao.list_page(
        ...
        enrichment_status=status, viewer=user.id)
