"""DEAD REFERENCE — the v0.1 SQLite schema (lineage closed at v10).

Moved verbatim from connect/storage/schema.py at port phase P2; consumed
ONLY by the one-shot ETL (P4). The live DDL home is
connect/storage/schema.py (PostgreSQL baseline).
"""
...
from connect.storage.schema import (  # noqa: F401 -- shared error hierarchy
    StorageError,
    StorageVersionError,
)
...
# ==============================================================================
# Legacy SQLite schema (v0.1 lineage, closed at v10) — TRANSITIONAL.
#
# The DAO layer and the live v0.1 system still run on SQLite until port
# phase P2; everything below is deleted at cutover (P5). Conventions
# (log_buster discipline): INTEGER PRIMARY KEY ids (rowid-backed), ISO-8601
# UTC TEXT timestamps, FTS5 external-content tables maintained by triggers,
# sqlite-vec virtual-table DDL executed only by knowledge/vector.py.
# ==============================================================================
...
# v10 (v0.2 Phase A — queue/registry seam): cancel_requested is the durable
# cooperative-cancellation flag (workers/registry.CancelToken reads it at
# stage/iteration checkpoints; still-queued jobs are CAS-finished outright).
_DDL_JOB = f"""
