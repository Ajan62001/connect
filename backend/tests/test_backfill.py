...
async def test_backfill_extracts_links_for_legacy_documents(
    ...
    await db.execute("DELETE FROM document_link")
    assert await link_dao.list_for_document(db, doc_id) == []

    counts = await backfill_links.run(settings, follow=False)
